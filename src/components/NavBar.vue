  ...
  <nav class="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <button @click="toggleDark" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-400 transition-colors">
        ...
        <RouterLink to="/cart" class="relative p-2 rounded-lg text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors">
          ...
          <span v-if="cart.totalItems > 0" class="absolute -top-1 -right-1 bg-indigo-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
            {{ cart.totalItems }}
          </span>
        </RouterLink>
        <button v-if="auth.isLoggedIn" @click="auth.logout(); router.push('/')"
        ...
        <RouterLink v-else to="/login" class="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">
          Login
        </RouterLink>
      </div>
    </div>
